<?php
// admin_announcements.php - Display only. POST handling is in admin.php top (before HTML output).
...
if (!empty($filter_type))     { $where_clauses[] = "a.type=?";        $params[] = $filter_type;     $types .= 's'; }
if (!empty($filter_priority)) { $where_clauses[] = "a.priority=?";    $params[] = $filter_priority; $types .= 's'; }
if ($filter_active !== '')    { $where_clauses[] = "a.is_active=?";   $params[] = (int)$filter_active; $types .= 'i'; }
if (!empty($search))          { $s = "%$search%"; $where_clauses[] = "(a.title LIKE ? OR a.content LIKE ?)"; $params[] = $s; $params[] = $s; $types .= 'ss'; }
$where_sql = $where_clauses ? 'WHERE ' . implode(' AND ', $where_clauses) : '';

$sql = "SELECT a.*, b.batch_name, c.full_name as coordinator_name,
...
.ann-btn-primary{padding:10px 20px;background:linear-gradient(135deg,var(--o5),var(--o4));color:#fff;border:none;border-radius:8px;font-size:.875rem;font-weight:600;cursor:pointer;text-decoration:none;display:inline-flex;align-items:center;gap:8px;transition:all .2s;box-shadow:0 2px 8px rgba(249,115,22,0.25);}
.ann-btn-primary:hover{transform:translateY(-2px);box-shadow:0 4px 12px rgba(249,115,22,0.35);}
.ann-btn-secondary{padding:8px 16px;background:var(--bg);color:var(--text2);border:1px solid var(--border);border-radius:8px;font-size:.82rem;font-weight:600;cursor:pointer;text-decoration:none;display:inline-flex;align-items:center;gap:6px;transition:all .2s;}
.ann-btn-secondary:hover{background:var(--card);border-color:var(--text3);}
.ann-btn-danger{padding:8px 16px;background:rgba(239,68,68,0.1);color:var(--red);border:1px solid rgba(239,68,68,0.3);border-radius:8px;font-size:.82rem;font-weight:600;cursor:pointer;text-decoration:none;display:inline-flex;align-items:center;gap:6px;transition:all .2s;}
.ann-btn-danger:hover{background:rgba(239,68,68,0.2);border-color:var(--red);}
.ann-btn-sm{padding:6px 12px;font-size:.75rem;}
...
.ann-badge{display:inline-flex;align-items:center;padding:4px 10px;border-radius:6px;font-size:.7rem;font-weight:700;text-transform:uppercase;letter-spacing:.3px;}
.ann-badge-urgent{background:rgba(239,68,68,0.12);color:#dc2626;}
.ann-badge-important{background:rgba(249,115,22,0.12);color:var(--o6);}
.ann-badge-normal{background:rgba(148,163,184,0.12);color:#64748b;}
.ann-badge-active{background:rgba(34,197,94,0.12);color:#16a34a;}
.ann-badge-inactive{background:rgba(148,163,184,0.12);color:#64748b;}
.ann-badge-type{background:rgba(59,130,246,0.12);color:#2563eb;}
...
<div class="ann-header">
    <h2><i class="fas fa-<?php echo $action==='edit'?'edit':'plus-circle'; ?>"></i>
        <?php echo $action==='edit'?'Edit':'Create'; ?> Announcement</h2>
    <a href="admin.php?tab=announcements" class="ann-btn-secondary"><i class="fas fa-arrow-left"></i> Back to List</a>
</div>
...
<form method="POST" action="admin.php">
    ...
    <div class="form-section">
        ...
        <div class="form-grid">
            <div class="form-group">
                ...
                <div class="help-text">Select a specific batch or leave blank for all</div>
            </div>
            <div class="form-group">
            ...
            <div class="form-group form-group-full">
                <div class="checkbox-group">
                    <input type="checkbox" name="target_all" id="ann-target-all" value="1"
                           <?php echo ($announcement['target_all']??1)?'checked':''; ?>>
                    <label for="ann-target-all">Target All Students (ignores batch/coordinator filters)</label>
                </div>
            </div>
            <div class="target-info">
                <strong><i class="fas fa-info-circle"></i> Targeting Logic:</strong>
                If "Target All" is checked, the announcement will be visible to all active students regardless of batch or coordinator.
                If unchecked, only students matching the selected batch/coordinator will see it.
            </div>
        </div>
    </div>
    ...
    <div class="form-section">
        ...
        <div class="checkbox-group">
            <input type="checkbox" name="is_active" id="ann-is-active" value="1"
                   <?php echo ($announcement['is_active']??1)?'checked':''; ?>>
            <label for="ann-is-active">Active (visible to students)</label>
        </div>
        <div class="help-text">Uncheck to hide this announcement without deleting it</div>
    </div>
    ...
    <div class="form-actions">
        ...
        <a href="admin.php?tab=announcements" class="ann-btn-secondary"><i class="fas fa-times"></i> Cancel</a>
    </div>
</form>
...
<div class="ann-header">
    ...
    <a href="admin.php?tab=announcements&action=create" class="ann-btn-primary">
        <i class="fas fa-plus-circle"></i> Create New
    </a>
</div>

<form method="GET" action="admin.php" class="filters-bar">
    ...
    <div class="filter-group">
        ...
        <button type="submit" class="ann-btn-secondary"><i class="fas fa-search"></i> Filter</button>
    </div>
    <?php if ($filter_type || $filter_priority || $filter_active !== '' || $search): ?>
    <div class="filter-group">
        <label class="filter-label">&nbsp;</label>
        <a href="admin.php?tab=announcements" class="ann-btn-secondary"><i class="fas fa-times"></i> Clear</a>
    </div>
    <?php endif; ?>
</form>
...
<div class="ann-table">
    <table>
        <thead>
            <tr>
                <th style="width:5%;">ID</th><th style="width:25%;">Title</th><th style="width:10%;">Type</th>
                <th style="width:10%;">Priority</th><th style="width:12%;">Target</th><th style="width:10%;">Read Stats</th>
                <th style="width:8%;">Status</th><th style="width:12%;">Created</th><th style="width:8%;">Actions</th>
            </tr>
        </thead>
        <tbody>
            ...
            <tr>
                ...
                <td><span class="ann-badge ann-badge-type"><?php echo htmlspecialchars($ann['type']); ?></span></td>
                <td><span class="ann-badge ann-badge-<?php echo $ann['priority']; ?>"><?php echo ucfirst($ann['priority']); ?></span></td>
                <td>
                    <?php if ($ann['target_all']): ?>
                        <span style="font-size:.82rem;"><i class="fas fa-users"></i> All Students</span>
                    <?php else: ?>
                        <?php if ($ann['batch_name']): ?><div style="font-size:.75rem;"><i class="fas fa-layer-group"></i> <?php echo htmlspecialchars($ann['batch_name']); ?></div><?php endif; ?>
                        <?php if ($ann['coordinator_name']): ?><div style="font-size:.75rem;"><i class="fas fa-user-tie"></i> <?php echo htmlspecialchars($ann['coordinator_name']); ?></div><?php endif; ?>
                        <?php if (!$ann['batch_name'] && !$ann['coordinator_name']): ?><span style="font-size:.82rem;color:var(--text3);">No filter</span><?php endif; ?>
                    <?php endif; ?>
                </td>
                <td>
                    <?php $rp = $ann['total_students']>0 ? round(($ann['read_count']/$ann['total_students'])*100) : 0; ?>
                    <div class="read-stat"><i class="fas fa-eye"></i><span><?php echo $ann['read_count']; ?>/<?php echo $ann['total_students']; ?></span></div>
                    <div style="font-size:.7rem;color:var(--text3);margin-top:2px;"><?php echo $rp; ?>% read</div>
                </td>
                <td><span class="ann-badge ann-badge-<?php echo $ann['is_active']?'active':'inactive'; ?>"><?php echo $ann['is_active']?'Active':'Inactive'; ?></span></td>
                <td style="font-size:.82rem;color:var(--text2);">
                    <?php echo date('M d, Y', strtotime($ann['created_at'])); ?>
                    <div style="font-size:.7rem;color:var(--text3);margin-top:2px;"><?php echo date('h:i A', strtotime($ann['created_at'])); ?></div>
                </td>
                <td>
                    <div class="action-btns">
                        <a href="admin.php?tab=announcements&action=edit&id=<?php echo $ann['id']; ?>"
                           class="ann-btn-secondary ann-btn-sm" title="Edit"><i class="fas fa-edit"></i></a>
                        <button type="button" class="ann-btn-danger ann-btn-sm"
                                onclick="annDelete(<?php echo $ann['id']; ?>)" title="Delete">
...
<div class="empty-state">
    ...
    <a href="admin.php?tab=announcements&action=create" class="ann-btn-primary" style="margin-top:16px;">
        <i class="fas fa-plus-circle"></i> Create Announcement
    </a>
</div>
<?php endif; ?>

<form method="POST" action="admin.php" id="annDeleteForm" style="display:none;">
    <input type="hidden" name="delete_announcement" value="1">
    <input type="hidden" name="announcement_id"     id="annDeleteId">
</form>

<script>
function annDelete(id) {
    if (!confirm('Are you sure you want to delete this announcement? This will also remove all read tracking data.')) return;
    document.getElementById('annDeleteId').value = id;
    document.getElementById('annDeleteForm').submit();
}
</script>
