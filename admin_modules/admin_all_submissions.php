<?php
// admin_all_submissions.php - Display only. POST handling is in admin.php top (before HTML output).
...
function buildSubUrl($params = []) {
    ...
    return 'admin.php?' . http_build_query(array_merge($base, $params));
}
?>
...
.ts-modal{display:none;position:fixed;inset:0;background:rgba(15,23,42,.6);z-index:9999;align-items:center;justify-content:center;padding:20px;backdrop-filter:blur(6px);}
.ts-modal.active{display:flex;}
.ts-modal-content{background:var(--card);border-radius:18px;width:100%;max-width:680px;max-height:92vh;overflow-y:auto;box-shadow:0 30px 80px rgba(0,0,0,.3);}
...
@media(max-width:768px){.ts-detail-grid{grid-template-columns:1fr;}.ts-stats{grid-template-columns:repeat(2,1fr);}.ts-section-body{padding:16px;}.ts-modal-content{border-radius:12px;}}
</style>
...
<div class="ts-section">
    <div class="ts-section-header">
        ...
        <form method="GET" action="admin.php" style="margin:0;">
            <input type="hidden" name="tab"         value="all_submissions">
    ...
    <div class="ts-section-body">
        ...
        <div class="ts-toolbar">
            ...
            <form method="GET" action="admin.php" style="margin:0;display:contents;">
                <input type="hidden" name="tab"         value="all_submissions">
                <input type="hidden" name="sub_status"  value="<?php echo htmlspecialchars($filterStatus); ?>">
                <input type="hidden" name="task_filter" value="<?php echo $filterTask; ?>">
        ...
        <!-- Bulk form -->
        <form method="POST" action="admin.php" id="allSubBulkForm">
            <input type="hidden" name="bulk_update_submissions" value="1">
                ...
                <table class="ts-table">
                    <thead>
                        <tr>
                            ...
                            <th>Student</th><th>Task</th><th>Submitted</th>
                            <th class="center">Links</th><th class="center">Points</th>
                    ...
                    <?php foreach ($submissions as $sub): ?>
                        <tr id="row-<?php echo $sub['id']; ?>">
                            ...
                            <td>
                                <?php
                                $statusIcons = ['submitted'=>'fa-paper-plane','under_review'=>'fa-magnifying-glass','approved'=>'fa-circle-check','rejected'=>'fa-circle-xmark','revision_requested'=>'fa-rotate','draft'=>'fa-pen'];
                            ...
                            <td>
                                <div class="ts-actions">
                                    <button type="button" class="ts-btn ts-btn-secondary ts-btn-sm"
                                            onclick='allSubOpenReview(<?php echo json_encode([
                                                'id'              => (int)$sub['id'],
                                                'full_name'       => $sub['full_name'] ?? '',
                                                'email'           => $sub['email'] ?? '',
                                                'task_title'      => $sub['task_title'] ?? '',
                                                'task_max_points' => (int)($sub['task_max_points'] ?? 0),
                                            ...
                                            ], JSON_HEX_TAG|JSON_HEX_APOS|JSON_HEX_QUOT|JSON_HEX_AMP); ?>)' title="View & Review">
                                        <i class="fas fa-eye"></i>
                                    </button>
                                    <?php if ($sub['status'] !== 'approved'): ?>
                                    <button type="button" class="ts-btn ts-btn-success ts-btn-sm"
                                            onclick="allSubQuickAction(<?php echo $sub['id']; ?>,'approved')" title="Approve">
                                        <i class="fas fa-check"></i>
                                    </button>
                                    <?php endif; ?>
                                    <?php if ($sub['status'] !== 'rejected'): ?>
                                    <button type="button" class="ts-btn ts-btn-danger ts-btn-sm"
                                            onclick="allSubQuickAction(<?php echo $sub['id']; ?>,'rejected')" title="Reject">
...
<!-- REVIEW MODAL -->
<div id="allSubReviewModal" class="ts-modal">
    <div class="ts-modal-content">
        <div class="ts-modal-header">
            <div class="ts-modal-title"><i class="fas fa-clipboard-check" style="color:var(--o5);"></i> Review Submission</div>
            <button type="button" class="ts-modal-close" id="allSubModalCloseBtn">&times;</button>
        </div>
        <form method="POST" action="admin.php" id="allSubReviewForm">
            <div class="ts-modal-body">
                <input type="hidden" name="update_submission_status" value="1">
                <input type="hidden" name="submission_id" id="allsub_rev_id">
                <div class="ts-detail-grid">
                    <div>
                        <div class="ts-detail-label"><i class="fas fa-user fa-xs"></i> Student</div>
                        <div class="ts-detail-value" id="allsub_student_name">—</div>
                        <div style="font-size:.75rem;color:var(--text3);" id="allsub_student_email"></div>
                    </div>
                    <div>
                        <div class="ts-detail-label"><i class="fas fa-tasks fa-xs"></i> Task</div>
                        <div class="ts-detail-value" id="allsub_task_title">—</div>
                        <div style="font-size:.75rem;color:var(--text3);" id="allsub_task_pts"></div>
                    </div>
                    <div>
                        <div class="ts-detail-label"><i class="fas fa-clock fa-xs"></i> Submitted At</div>
                        <div class="ts-detail-value" id="allsub_submitted_at">—</div>
                    </div>
                    <div>
                        <div class="ts-detail-label"><i class="fas fa-circle-dot fa-xs"></i> Current Status</div>
                        <div id="allsub_current_status"></div>
                    </div>
                </div>
                <div id="allsub_text_section" style="display:none;margin-bottom:16px;">
                    <div class="ts-detail-label" style="margin-bottom:6px;"><i class="fas fa-align-left fa-xs"></i> Submission Text</div>
                    <div class="ts-submission-content" id="allsub_submission_text"></div>
                </div>
                <div id="allsub_links_section" style="display:none;margin-bottom:16px;">
                    <div class="ts-detail-label" style="margin-bottom:8px;"><i class="fas fa-link fa-xs"></i> Submission Links</div>
                    <div id="allsub_links_container" style="display:flex;flex-direction:column;gap:6px;"></div>
                </div>
                <div id="allsub_prev_feedback_wrap" style="display:none;margin-bottom:16px;">
                    <div class="ts-detail-label" style="margin-bottom:6px;"><i class="fas fa-comment-dots fa-xs"></i> Previous Feedback</div>
                    <div id="allsub_prev_feedback" style="background:var(--bg);border:1px solid var(--border);border-radius:9px;padding:12px 14px;font-size:.84rem;color:var(--text2);line-height:1.55;"></div>
                </div>
                <hr class="ts-divider">
                <div class="ts-form-group">
                    <label class="ts-form-label">Update Status <span class="req">*</span></label>
                    <select name="status" id="allsub_status" class="ts-form-select" onchange="allSubTogglePoints()">
                ...
                <div class="ts-form-group" id="allSubPointsGroup" style="display:none;">
                    <label class="ts-form-label">Points to Award <span class="req">*</span></label>
                    <input type="number" name="points_earned" id="allsub_points" class="ts-form-input" min="0" max="9999" placeholder="Enter points…">
                    <div class="ts-form-hint"><i class="fas fa-info-circle"></i> Max points for this task: <strong id="allsub_max_pts">—</strong></div>
                </div>
                <div class="ts-form-group">
                    <label class="ts-form-label">Feedback / Comments</label>
                    <textarea name="feedback" id="allsub_feedback" class="ts-form-textarea" placeholder="Provide constructive feedback…"></textarea>
                    <div class="ts-form-hint"><i class="fas fa-bell fa-xs"></i> This will be sent to the student as a notification.</div>
                </div>
            </div>
            <div class="ts-modal-footer">
                <button type="button" class="ts-btn ts-btn-secondary" id="allSubModalCancelBtn"><i class="fas fa-times"></i> Cancel</button>
...
<!-- Quick-action form -->
<form method="POST" action="admin.php" id="allSubQuickForm" style="display:none;">
    <input type="hidden" name="update_submission_status" value="1">
    <input type="hidden" name="submission_id" id="allsub_qa_id">
    <input type="hidden" name="status"         id="allsub_qa_status">
    <input type="hidden" name="feedback"       value="">
</form>

<script>
(function(){
    function allSubClose() { document.getElementById('allSubReviewModal').classList.remove('active'); }

    window.allSubOpenReview = function(sub) {
        document.getElementById('allsub_rev_id').value        = sub.id;
        document.getElementById('allsub_student_name').textContent  = sub.full_name  || '—';
        document.getElementById('allsub_student_email').textContent = sub.email      || '';
        document.getElementById('allsub_task_title').textContent    = sub.task_title || '—';
        document.getElementById('allsub_task_pts').textContent      = sub.task_max_points ? 'Max '+sub.task_max_points+' pts' : '';
        document.getElementById('allsub_max_pts').textContent       = sub.task_max_points || '—';
        document.getElementById('allsub_points').max                = sub.task_max_points || 9999;

        var d = new Date(sub.submitted_at);
        document.getElementById('allsub_submitted_at').textContent =
        ...
        document.getElementById('allsub_current_status').innerHTML =
        ...
        document.getElementById('allsub_status').value  = sub.status;
        document.getElementById('allsub_points').value  = sub.points_earned || '';
        document.getElementById('allsub_feedback').value = '';
        allSubTogglePoints();

        var txtSec = document.getElementById('allsub_text_section');
        if (sub.submission_text && sub.submission_text.trim()) {
            document.getElementById('allsub_submission_text').textContent = sub.submission_text;
            txtSec.style.display = 'block';
        } else { txtSec.style.display = 'none'; }

        var linksSec = document.getElementById('allsub_links_section');
        var linksCon = document.getElementById('allsub_links_container');
        linksCon.innerHTML = '';
        var links = [];
        ...
        var pfWrap = document.getElementById('allsub_prev_feedback_wrap');
        if (sub.feedback && sub.feedback.trim()) {
            document.getElementById('allsub_prev_feedback').textContent = sub.feedback;
            pfWrap.style.display = 'block';
        } else { pfWrap.style.display = 'none'; }

        document.getElementById('allSubReviewModal').classList.add('active');
    };

    window.allSubTogglePoints = function() {
        var s = document.getElementById('allsub_status').value;
        var pg = document.getElementById('allSubPointsGroup');
        var pi = document.getElementById('allsub_points');
        pg.style.display = s === 'approved' ? 'block' : 'none';
        pi.required      = s === 'approved';
    };

    window.allSubQuickAction = function(id, status) {
        ...
        document.getElementById('allsub_qa_id').value     = id;
        document.getElementById('allsub_qa_status').value = status;
        document.getElementById('allSubQuickForm').submit();
    };

    document.getElementById('allSubModalCloseBtn').addEventListener('click', allSubClose);
    document.getElementById('allSubModalCancelBtn').addEventListener('click', allSubClose);
    document.getElementById('allSubReviewModal').addEventListener('click', function(e){ if(e.target===this) allSubClose(); });
    ...
    window.confirmBulk = function() {
        ...
        var action = document.querySelector('#allSubBulkForm select[name="bulk_status"]').value;
